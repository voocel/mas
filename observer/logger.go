...
func (o *LoggerObserver) OnLLMStart(ctx context.Context, state *runner.State, req *llm.Request) {
	o.logger.Printf("llm start agent=%s turn=%d messages=%d", state.Agent.Name(), state.Turn, len(state.Messages))
}

func (o *LoggerObserver) OnLLMEnd(ctx context.Context, state *runner.State, resp *llm.Response, err error) {
	if err != nil {
		o.logger.Printf("llm error agent=%s turn=%d err=%v", state.Agent.Name(), state.Turn, err)
		return
	}
	contentLen := 0
	if resp != nil {
	...
	o.logger.Printf("llm end agent=%s turn=%d content_len=%d", state.Agent.Name(), state.Turn, contentLen)
}
...
func (o *LoggerObserver) OnToolCall(ctx context.Context, state *runner.ToolState) {
	...
	o.logger.Printf("tool call name=%s id=%s", state.Call.Name, state.Call.ID)
}
...
func (o *LoggerObserver) OnToolResult(ctx context.Context, state *runner.ToolState) {
	...
	if err != "" {
		o.logger.Printf("tool result id=%s err=%s", state.Result.ID, err)
		return
	}
	o.logger.Printf("tool result id=%s size=%d", state.Result.ID, len(state.Result.Result))
}
